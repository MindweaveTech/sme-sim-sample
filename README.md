# Australian SME Business Dataset (Free Sample)

A realistic, relational business dataset generated by simulating an Australian retail SME day-by-day over 2 financial years. Every transaction flows through double-entry accounting. Not random data — simulated business operations.

**Free to use for development, testing, training, and evaluation.**

> Full dataset: 42 tables, 83,000+ rows, CSV + SQL + PostgreSQL dump — [$49 on Gumroad](https://mindweavetech.gumroad.com/l/trcdsq) | [Free on Kaggle (sample)](https://www.kaggle.com/datasets/mindweavetech/australian-sme-business-dataset)

---

## Quick Start

```bash
# Clone and explore
git clone https://github.com/MindweaveTech/sme-sim-sample.git
cd sme-sim-sample

# Check what's inside
cat _row_counts.csv

# Load into SQLite and query
sqlite3 :memory: <<'SQL'
.mode csv
.import companies.csv companies
.import chart_of_accounts.csv chart_of_accounts
.import sales_orders_sample.csv sales_orders
.import journal_entries_sample.csv journal_entries
SELECT count(*) as total_sales FROM sales_orders;
SELECT account_type, count(*) as accounts FROM chart_of_accounts GROUP BY account_type;
SQL
```

## What is this?

Unlike flat fake-data generators (Faker, Mockaroo), this dataset was built by running a **day-by-day business simulation**. A simulated company opens for business on 1 July 2023, hires staff, buys inventory, makes sales, runs payroll, lodges tax returns — for 2 full financial years. Relationships emerge naturally, not by scripting.

**This sample includes ~2,800 rows across 26 tables.** Full reference tables plus sampled transaction tables.

## What makes this different from AdventureWorks / Northwind / Faker

| Feature | AdventureWorks | Northwind | Faker/Mockaroo | **sme-sim** |
|---------|---------------|-----------|----------------|-------------|
| Cross-domain traceability | Partial | No | No | **Full** — sale → invoice → payment → bank → journal entry |
| Double-entry accounting | No | No | No | **Yes** — debits always equal credits |
| Tax compliance | US-only | None | None | **Australian** — ATO PAYG, GST, BAS, super |
| Temporal realism | Static | Static | Random | **Simulated** — seasonal patterns, staff turnover, payment behaviour |
| Relational integrity | Good | Basic | None | **44 FK relationships**, all enforced |
| Still maintained | Last updated 2014 | Last updated ~2000 | N/A | **2025** |

## Tables included

### Full reference tables
| Table | Rows | Description |
|-------|------|-------------|
| companies | 1 | Simulated business entity |
| departments | 4 | Management, Sales, Warehouse, Administration |
| chart_of_accounts | 56 | Full Australian retail chart of accounts |
| products | 25 | Outdoor/camping retail products |
| customers | 173 | Customers with payment terms and behaviour traits |
| suppliers | 10 | Stock and service suppliers |
| employees | 14 | Full employee records |
| bank_accounts | 2 | Operating and savings accounts |
| fixed_assets | 10 | POS, computers, shop fittings, vehicle |
| projects | 10 | Internal projects |
| tax_returns | 8 | Quarterly BAS returns |
| tax_return_lines | 56 | BAS line items |
| pay_runs | 52 | Fortnightly pay runs |
| bank_statements | 24 | Monthly statements |
| price_history | 25 | Price changes |

### Sampled transaction tables (first N rows)
| Table | Sample Rows | Full Dataset |
|-------|------------|-------------|
| sales_orders | 200 | 2,707 |
| sales_order_lines | 500 | 12,189 |
| sales_invoices | 200 | 2,707 |
| journal_entries | 200 | 7,407 |
| journal_entry_lines | 500 | 18,623 |
| bank_transactions | 200 | 3,565 |
| inventory_movements | 200 | 13,135 |
| pay_run_lines | 100 | 621 |
| purchase_orders | 100 | 964 |

## Who this is for

- **Developers** building ERP, accounting, or business software
- **QA teams** testing complex business workflows across modules
- **Consultants** running demos and training without exposing client data
- **Data engineers** building ETL pipelines against a realistic source
- **Students** studying business systems and accounting
- **AI/ML teams** needing realistic business data for training

## Simulated business

| Field | Value |
|-------|-------|
| Company | Outback Outdoor Supplies Pty Ltd |
| Industry | Retail (outdoor/camping equipment) |
| Location | Western Australia |
| Period | 1 July 2023 – 30 June 2025 |
| Employees | 14 (12 active, 2 terminated) |
| Customers | 173 |
| Products | 25 SKUs |

## Foreign key relationships

See `_relationships.csv` for the complete FK map showing how all 42 tables connect in the full dataset.

## Full dataset & other products

| Product | Tables | Rows | Price |
|---------|--------|------|-------|
| [Free Sample (this repo)](https://mindweavetech.gumroad.com/l/yneyf) | 26 | ~2,800 | Free |
| [Complete SME Dataset](https://mindweavetech.gumroad.com/l/trcdsq) | 42 | 83,000+ | $49 |
| [Multi-Company Bundle (3 companies)](https://mindweavetech.gumroad.com/l/lnulcg) | 42 × 3 | 246,000+ | $99 |
| [Enterprise Pack (5 companies)](https://mindweavetech.gumroad.com/l/olgtyb) | 42 × 5 | 400,000+ | $199 |
| Domain packs (Accounting, Sales, HR, Inventory) | 10-15 each | varies | $19 each |

Also available on [Kaggle](https://www.kaggle.com/datasets/mindweavetech/australian-sme-business-dataset).

## License

This sample is free to use for development, testing, training, and evaluation. The full dataset is commercially licensed — see [Gumroad](https://mindweavetech.gumroad.com) for terms.

---

Built by [Mindweave Technologies](https://mindweave.tech)
